...
>
    <Icon icon={expanded ? 'lucide:chevrons-left' : 'lucide:chevrons-right'} width="18" />
    {#if expanded}
...
<div class="lg:hidden fixed bottom-6 left-1/2 -translate-x-1/2 z-50">
  {#if mobileOpen}
    <div
      class="absolute bottom-full left-1/2 -translate-x-1/2 mb-3 flex flex-col items-stretch gap-1 rounded-2xl border border-text-muted/15 bg-surface/95 backdrop-blur-md p-2 shadow-xl min-w-40"
...
<a
  ...
  <button
    ...
    class="w-14 h-14 rounded-full border border-text-muted/15 bg-surface/90 backdrop-blur-md shadow-lg flex items-center justify-center text-text-muted hover:text-[var(--nav-accent-fn)] transition-colors"
